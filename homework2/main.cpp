...
void PolynomialOperators() {
  stringstream ss;
  const auto a =  Polynomial(0, 2, new int[3]{1, 2, 3});  // 3x^2+2x+1
  const auto b = Polynomial(1, 2, new int[2]{-3, 4});     // 4x^2-3x
  auto c = Polynomial(-1, 2, new int[4]{1, -3, 0, -1});   // -x^2-3+x^-1
  const auto d = Polynomial();
  const auto e =  Polynomial(0, 2, new int[3]{0, 3, -3});
  const auto f = Polynomial(-2, 2, new int[5]{1, 0, 0, 0, 1});
  assert(a * b == b * a); // 12x^4 - x^3 - 2x^ 2 - 3x
  assert(d * e == d);
  assert(a * b * c == a * (b * c));
  assert(a * b == Polynomial(1, 4, new int[4]{-3, -2, -1, 12}));
  assert(a + b == b + a);
  assert(d + e == e);
  assert(a + b + c == a + (b + c)); //6x^2
  assert(5 * a == a * 5);
